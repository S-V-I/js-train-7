...
// Функція createSymbolForKey створює новий символ за допомогою
// Symbol.for() та повертає його
function createSymbolForKey(key) {
  // Використовуємо Symbol.for() для створення нового символу symbol
  // зі значенням key
  // Symbol.for() шукає символ в глобальному реєстру символів. Якщо
  // символ з таким ключем вже існує, він повертає його, в іншому випадку
...
// Функція compareSymbolsForKey порівнює два символи за допомогою
// оператора '==='
...
// Функція getSymbolKey приймає символ як аргумент та повертає ключ,
// який був використаний для його створення
...
let symbolForKey = Symbol.for("myKey"); // Створюємо новий символ або
// отримуємо існуючий з глобального реєстру символів
console.log(getSymbolKey(symbolForKey)); // Виводимо ключ символу.
// Виведе: myKey
...
function useSymbolToStringTag() {
  ...
  // Встановлюємо властивість Symbol об'єкту myObject
  // на значення "CustomObject".
...
function useSymbolDescription() {
  ...
  // Отримуємо опис символу mySymbol в змінну symbolDescription
...
let rangeObject = {
  // Використовуємо Symbol.iterator для створення ітератора всередині
  // об'єкта "myObject"
  ...
  // що містить поточне значення,
  // та не забуваємо збільшити індекс за допомогою інкремент,
  // і "done" - false, означаючи, що ітерація ще не закінчена.
  // Якщо індекс вийшов за межі масиву ключів, повертаємо об'єкт
  // з властивістю "done" - true, означаючи, що ітерація закінчена
...
// Функція "useSymbolIterator" використовує ітератор для отримання
// значень об'єкта
...
let myObject = {
  ...
  //Створюємо властивість currentIndex зі значенням 0,
  // яка буде нашим лічильником в ітерації

  // Оголошення методу Symbol.iterator для об'єкта "myObject"
  //Повертаємо this

  // Оголошення методу "next" для ітерації
  // Створюємо логічний оператор який буде перевіряти чи властивість
  // об'єкту currentIndex менша ніж довжина масиву category
  // Створюємо змінну value якій присвоємо властивість name елемента
  // масиву category з індексом currentIndex
  // Збільшимо currentIndex на одиницю
  // Повертаємо об'єкт з властивістю value значенням якої буде value,
  // та прапорцем done: false
  // Якщо властивість об'єкту currentIndex більше або дорівнює довжині
  // масиву category повертаємо об'єкт з прапорцем done: true,
  // коли ітерація закінчена
...
//Виведе [ 'apple', 'banana', 'orange', 'grape', 'mango' ]
